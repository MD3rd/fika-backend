...
var con = mysql.createConnection({
  ...
  user: "je296k5uuds1srlg",
  password: "n5ah4rqh5il7b817",
  database: "xy9m76v9rxhak8jk"
});
...
app.post('/register', (req, res) => {
  const sql = "INSERT INTO users (`username`, `salt`, `password`) VALUES (?, 'Disciple', ?)";
  ...
  const hashpassword = CryptoJS.MD5('Disciple'+password).toString();

  // เพิ่มข้อมูลในฐานข้อมูล
  con.query(sql, [username, hashpassword], (err, data) => {
    if (err) {
...
/*app.post('/api/save-avatar', upload.single('avatar'), (req, res) => { 
  ...
  con.beginTransaction((err) => { 
    ...
    con.query(query, [avatar, user_id], (err, results) => { 
